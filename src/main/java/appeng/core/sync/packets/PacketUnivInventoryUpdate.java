...
public class PacketUnivInventoryUpdate extends AppEngPacket {
    ...
    // input.
    @Nullable
    private final List<IExAEStack<?>> list;
    // output...
    ...
    // automatic.
    public PacketUnivInventoryUpdate(final ByteBuf stream) throws IOException {
        this.data = null;
        ...
        try (GZIPInputStream gzReader = new GZIPInputStream(new InputStream() {
            ...
            while (uncompressed.readableBytes() > 0) {
                this.list.add(AEApi.instance().storage().readExStackFromPacket(uncompressed));
            }
        }
    ...
    // api
    public PacketUnivInventoryUpdate() throws IOException {
        this((byte) 0);
    }

    // api
    public PacketUnivInventoryUpdate(final byte ref) throws IOException {
        this.ref = ref;
        ...
        this.compressFrame = new GZIPOutputStream(new OutputStream() {
            ...
            public void write(final int value) throws IOException {
                PacketUnivInventoryUpdate.this.data.writeByte(value);
            }
        });
    ...
    public <T extends IAEStack<T>> void appendItem(final T is) throws IOException, BufferOverflowException {
        appendItem(ExAEStack.of(is));
    }

    public void appendItem(final IExAEStack<?> is) throws IOException, BufferOverflowException {
